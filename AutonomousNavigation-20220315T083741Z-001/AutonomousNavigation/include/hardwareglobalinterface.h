...
#define PREDICTION_TIMEOUT 200
//#define PEOPLENEIGH_TIMEOUT 200
...
public:
  ...
  static HardwareGlobalInterface &initialize(HardwareParameters* hp){
    ...
    instances.back()->subPrediction.reset(new ZMQCommon::Subscriber());
    instances.back()->subPrediction->register_callback([&](const char *topic, const char *buf, size_t size, void *data){instances.back()->sub_prediction(topic,buf,size,data);});
    instances.back()->subPrediction->start(hp->predictionPublisher,"PUB_PRED");
    
  //  instances.back()->subPeopleNeigh.reset(new ZMQCommon::Subscriber());
  //  instances.back()->subPeopleNeigh->register_callback([&](const char *topic, const char *buf, size_t size, void *data){instances.back()->sub_peopleNeigh(topic,buf,size,data);});
  //  instances.back()->subPeopleNeigh->start(hp->peopleNeighPublisher,"PUB_PEOPLENEIGH");
  ...
  bool getPredictionData(std::vector<People> & predictionData, double & robotVel) {
    std::unique_lock<std::mutex> lock(predictionMTX);
    double currTime = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
    robotVel = this->robotVel;
    predictionData = this->predictionData;
    if(currTime - this->predictionTimer < PREDICTION_TIMEOUT){
      return true;
    }else{
      return false;
    }
...
/*
  bool getPeopleNeighData(std::vector<People> & peopleNeighData) {
    std::unique_lock<std::mutex> lock(peopleNeighMTX);
    double currTime = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
    peopleNeighData = this->peopleNeighData;
    if(currTime - this->peopleNeighTimer < PEOPLENEIGH_TIMEOUT){
      return true;
    }else{
      return false;
    }
...
private:
  ...
  std::mutex predictionMTX;
  std::vector<People> predictionData;
  double robotVel;
  double predictionTimer;
  std::unique_ptr<ZMQCommon::Subscriber> subPrediction;
  /*
  std::mutex peopleNeighMTX;
  std::vector<People> peopleNeighData;
  double peopleNeighTimer;
  std::unique_ptr<ZMQCommon::Subscriber> subPeopleNeigh;
*/
  ...
  void sub_prediction(const char *topic, const char *buf, size_t size, void *data);
  //void sub_peopleNeigh(const char *topic, const char *buf, size_t size, void *data);

  HardwareParameters* params = nullptr;
  double safetyWidth = 0.1;
  double criticalDistance = 0.1;
