...
void HardwareGlobalInterface::sub_prediction(const char *topic, const char *buf, size_t size, void *data){
  nlohmann::json j;
  std::vector<People> people;
  try {
    j = nlohmann::json::parse(std::string(buf, size));
    double vel = j.at("control_inputs");
    //std::cout << j.dump() << std::endl;
    nlohmann::json j_data = j.at("object_predicted");
    people.clear();
    ...
    for (nlohmann::json::iterator it = j_data.begin(); it != j_data.end(); ++it) {
	People partypeople;
        partypeople.Xpred.push_back( (*it).at("xpred").get<std::vector<double>>() ); 
        partypeople.Xpred.push_back( (*it).at("ypred").get<std::vector<double>>() ); 
        people.push_back(partypeople);
      }
    {
      std::unique_lock<std::mutex> lockPrediction(predictionMTX);
      predictionData = people;
      robotVel = vel;
      predictionTimer = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
...
/*void HardwareGlobalInterface::sub_peopleNeigh(const char *topic, const char *buf, size_t size, void *data){
  nlohmann::json j;
  ...
  try {
    ...
    //double vel = j.at("control_inputs");
    //std::cout << j.dump() << std::endl;
    nlohmann::json j_data = j.at("peopleNeigh");
    peopleNeigh.clear();
    ...
    for (nlohmann::json::iterator it = j_data.begin(); it != j_data.end(); ++it) {
        People partypeople;
        partypeople.x.push_back( (*it).at("x").get<std::vector<double>>() ); 
        partypeople.y.push_back( (*it).at("y").get<std::vector<double>>() );
        partypeople.vx.push_back( (*it).at("vx").get<std::vector<double>>() ); 
        partypeople.vy.push_back( (*it).at("vy").get<std::vector<double>>() );
        partypeople.id = (*it).at("id");
 
        peopleNeigh.push_back(partypeople);
      }
    {
      std::unique_lock<std::mutex> lockPeopleNeigh(peopleNeighMTX);
      peopleNeighData = peopleNeigh;
      peopleNeighTimer = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
...
void HardwareGlobalInterface::sub_frontLidar(const char *topic, const char *buf, size_t size, void *data)
{
  ...
  std::vector<RobotStatus::LidarDatum> filtered;
  const auto & datum = frontLidar.datum;
  for (int i=0; i<datum.size(); ++i) {
    const auto & d1 = datum[i];
    for (int j=0; j<datum.size(); ++j) {
      const auto & d2 = datum[j];
      if (i!=j && std::hypot(d1.x-d2.x, d1.y-d2.y) < 0.1) {
        filtered.push_back(d1);
        break;
      }
    }
  }
  frontLidar.datum = filtered;
}
...
void HardwareGlobalInterface::vehicleSafeMove(float vel, float omega)
{
  ...
  vehicleMove(vel*brakingCoefficient,omega);
